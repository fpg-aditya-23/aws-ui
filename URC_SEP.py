...
# 🔹 CONFIG
CONFIG = {
    "bucket": "ing-urban-resort-concepts-useast-prod",
    "prefix": "SEP/SES/",
...
def download_data(location, date_input, base_path, logger):
    ...
    try:
        ...
        # Folder Structure
        # Base Path / URC(SEP) / Date / Location
        base_folder = os.path.join(base_path, "URC(SEP)", date_input)
        os.makedirs(base_folder, exist_ok=True)

        not_found_dir = os.path.join(base_folder, "URC_NOT_FOUND")
        os.makedirs(not_found_dir, exist_ok=True)

        logger(f"⬇ Starting URC(SEP) → {location}")
        ...
        logger(
            f"✅ URC(SEP) Done → Downloaded: "
            f"{download_count}, Sorted: {sorted_count}"
        )
